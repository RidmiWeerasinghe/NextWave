...
function SearchBar() {
    const [{ user, accessToken, searchedArtists, searchResultsLoading }, dispatch] = useStateValue()
    ...
    const isPlaylistPage = location.pathname === '/singleplaylist' || location.pathname === '/searchresultstracks' || location.pathname === '/' || location.pathname === '/callback'
    const isUserPlaylistPage = location.pathname === '/myplaylists'

    console.log("isUserPlaylistPage")
    console.log(isUserPlaylistPage)
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            //searching a user created playlist
            if (isUserPlaylistPage) {
                console.log("searching a playlist...")

                // search request to the server
                axios.get(`http://localhost:5555/playlist/search/?email=${user.email}&searchtext=${inputText}`)
                    .then(response => {
                        dispatch({
                            type: 'SET_SEARCHEDUSERPLAYLIST',
                            searchedUserPlaylist:response.data
                        })
                        dispatch({
                            type: 'SET_SEARCHRESULTLOADING',
                            searchResultsLoading: false
                        })
                        navigate('/searchresultsmyplaylists')
                    })
                    .catch(err => {
                        console.log(err)
                    })
        ...
        <section className="flex items-center searchBarContainer  gap-4 w-96 max-md:w-full rounded-3xl border border-lightTextColor">
            ...
            >
                <input
                    ...
                    placeholder={isArtistsPage && "Type artist's name here" || isAlbumsPage && "Type album name here" || isPlaylistPage && "Type song name here" || isUserPlaylistPage && "Type playlist name here"}
